...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32, frame_inc=1, save_csv=None):
        ...
        #and x.split('.')[0].endswith("F")
        videos = [x for x in files if (x.split('.')[-1].lower() in VID_FORMATS and "F.mp4" in x and (save_csv is None or not (".".join(x.split("\\")[-1].split(".")[:-1]) + ".csv") in os.listdir(save_csv)))]
        print(videos)
        print(os.listdir(save_csv))
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
          ...
          while ret_val is None:
            if self.frame % self.frame_inc == 0:
              # Read video
              self.mode = 'video'
